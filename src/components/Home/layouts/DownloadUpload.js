...
import { AirplayOutlined, SyncOutlined, /*FolderOutlined,*/ OpenInBrowserOutlined } from '@material-ui/icons';
...
export default function StreamHLS({ history, handleError }) {
    ...
    const handleCreateStream = (e) => {
        ...
        setErrors(false);
        setLoading(true);
        Streams.createDownloadUploadStream(streamToken, /*recordingFolder,*/ videoURL, keepFiles).then(()=>history.push('/stream/'+streamToken)).catch(handleError).finally(() => setLoading(false));
    }
    ...
    return <>
        ...
        <Container maxWidth='lg' style={{ height: '100%', display: 'flex', flexDirection: 'column', paddingTop: 20 }}>
            <div style={{ display: 'flex', alignItems: 'center' }}>
                <SyncOutlined /> <Typography variant="h6" style={{ paddingLeft: 10 }}>Download/Upload</Typography>
            </div>
            <Container maxWidth='lg' style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
                <Typography variant="body1" style={{ flex: 1 }}>
                    Download YouTube video and upload to Sia Skynet
                </Typography>
                ...
                <form style={{ display: 'flex', flexDirection: 'column', flex: 9 }}>
                    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, justifyContent: 'space-around' }}>
                        ...
                        <FlavouredInput value={videoURL} onChange={setVideoURL} error={errors ? errors.url : false} label="Video source URL" tooltip="What do you want to upload? Copy the URL here, for example: https://www.youtube.com/watch?v=dQw4w9WgXcQ" />
                    ...
                    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'flex-end', marginBottom: 30, width: '100%', flex: 1 }}>
                        <Button type="submit" color='primary' onClick={handleCreateStream} startIcon={<AirplayOutlined />} variant="outlined" size="large">Start Upload</Button>
                    </div>
                </form>
            </Container>
        </Container>
